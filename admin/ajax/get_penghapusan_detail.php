...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['skpd_name'])) {
    ...
    // Siapkan query untuk mengambil data detail dari tabel penghapusan
    $query = "SELECT kode_barang, nama_barang, alasan, jumlah_barang, nilai_perolehan 
              FROM penghapusan 
    ...
    if ($stmt) {
        ...
        $output = '<h4 class="detail-title">Rincian Barang Usulan Penghapusan: ' . htmlspecialchars($skpd_name) . '</h4>';
        ...
        $output .= '<thead>
                        <tr>
                            ...
                            <th>Kode Barang</th>
                            <th>Nama Barang</th>
                            <th>Alasan Rencana Penghapusan</th>
                            <th>Jumlah</th>
        ...
        if ($result->num_rows > 0) {
            ...
            $output .= '<tr><td colspan="6" class="text-center">Tidak ada data rincian yang ditemukan.</td></tr>';
        }
...
// Menambahkan style khusus untuk tabel detail (sama seperti sebelumnya)
echo '<style>
